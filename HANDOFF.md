...
CHANGELOG(변경 이력) → ../AI비용-빈영역-오픈소스-기획서.md(v0.2+ 근거) →
...
## 2. 다음 기능 사이클: v0.2 (기획서 §3-B 범위)

**시작 전 아키텍처 과제가 먼저** — 2차 리뷰의 구조 지적:

- [ ] **어댑터 인터페이스 추출**: `transcripts.py`가 Claude 전용 단일체. "detect + record→events"
      인터페이스로 쪼개지 않으면 v0.2 어댑터(Codex/Gemini) 추가 시 인게스트 루프가 포크됨.
      세션 키도 `sessionId` 단독이라 에이전트 네임스페이스 필요.
...
- [ ] v0.3: M5 기반 배치 스케줄 조언, M6 개인 라우팅 힌트(옵트인 리플레이), M11 인간 수정 시간
